...
import { getUsersFailure, getUsersSuccess, userReset } from '../reducers/userReducer';
...
export const getUsers = createAsyncThunk('user/getUsers', async (_, { dispatch }) => {
  try {
    ...
    dispatch(updateAlert({ place: 'tc', message: 'Users fetched successfully', type: 'success' }));
    return true;
  } catch (err) {
